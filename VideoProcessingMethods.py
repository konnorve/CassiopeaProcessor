...
def initialize_params(files, startingFrameNum):
    ...
    for i, peak in enumerate(peaksOnBinaryImage):
        ...
        centroidRegion, onEdge = im.findJellyRegion(binaryCentroidDiff)
        ...
        if centroidRegion:
            ...
            biggestRegion = im.findJellyRegion(binaryDiffImg)[0]
        ...
        while i < framesInChunk:
            ...
            if isDownturn:
                ...
                if peak + peak2InflectionDiff >= 0 and peak + peak2TroughDiff < framesInChunk:
                    ...
                    elif onEdge: # no centroid region but on edge

                        data.append([peak + lastFrameOfPreviousChunk, peak, np.nan, np.nan, np.nan])

                        i += postPeakRefractoryPeriod
                        counter += 1

                    else: # no centroid region and not on edge
                        pass
